...
sub vcl_init {
	...
	#MoegirlphpGroup.add_backend(MainPHP);
    MoegirlphpGroup.add_backend(hhvm);
}
...
#set req.http.X-Forwarded-For = client.ip;
	...
	#判断转发给哪台机子
		#非zh m bits.moegirl.org的请求php
		if (! (req.http.host ~ "zh\.moegirl\.org" || req.http.host ~ "m\.moegirl\.org" || req.http.host ~ "bits\.moegirl\.org") ){  
			set req.backend_hint = MainPHP;
			
		#如果跟上传有关php，special页面（上传图片，恢复被删除页面，Special:移动页面/File:），所有上传的文件，url里带upload和delete的（delete也有页面删除，鉴于删除少全转php大概没问题）
...
sub vcl_synth {
	if(resp.status == 751) {
		set resp.http.Location = "//m.moegirl.org" + req.url;
		set resp.status = 302;
		return(deliver);
	}
}
